...
printTempInfo() {
    ...
    GPUTemp=$(sensors | grep -A 2 'radeon' | sed -n 's/.*+\([0-9]\+\).*/\1/p')
...
printMpdInfo() {
    MPDON=$(ps -A | grep -c mpd)
        ...
        echo -n "^fg()| ^ca(1,$MPD_RAND_CMD)RANDOM^ca() ^fg()$MpdRandom "
        echo -n "^fg()| ^ca(1,$MPD_TOGGLE_CMD)^ca(4,$MPD_NEXT_CMD)^ca(5,$MPD_PREV_CMD)MPD^ca()^ca()^ca() $MpdInfo"
    fi
    return
}

printDateInfo() {
    echo -n "^ca(1,$CAL_CMD)^fg()$(date '+%Y^fg($COLOR_SEP).^fg()%m^fg($COLOR_SEP).^fg()%d/^fg($DZEN_FG2)%a ^fg($COLOR_SEP)| ^fg()%H^fg($COLOR_SEP):^fg()%M^fg($COLOR_SEP):^fg()%S')^ca() "
    return
}
